...
function ModuleCard({ title, status, link, lastUpdated}) {
    ...
    return (
        <div className={`flex bg-white py-4 px-6 rounded-lg shadow-md justify-between items-center ${isOpen ? 'bg-[#4DD7BE] text-white' : 'bg-white text-slate-800'}`}>
            {/* Module content */}
            <a href={link} target="_blank" rel="noopener noreferrer" className="flex flex-col w-fit">
            ...
            <div className="flex w-fit gap-4">
                {isOpen ? (
                    ...
                    <a className={`text-sm flex justify-center m-auto py-3 px-5 rounded-lg shadow-md cursor-pointer ${isOpen ? 'bg-white text-slate-800' : 'bg-[#4DD7BE] text-white'}`} href={link}>
                        Open
                    </a>
                )}
            </div>
